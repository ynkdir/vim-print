...
function! print#syntax#dump(outfile, ...)
...
function! s:dump()
  ...
  for lnum in range(1, line('$'))
    ...
    for [str, attr] in synline
      if attr != curattr
        if !empty(textbuf)
          call add(out, s:text(join(textbuf, '')))
          let textbuf = []
        endif
        let curattr = attr
        call add(out, s:highlight(attr))
      endif
      call add(textbuf, str)
    endfor
    if !empty(textbuf)
      call add(out, s:text(join(textbuf, '')))
    endif
...
function! s:synattr(id)
  if has_key(s:syncache, a:id)
    return s:syncache[a:id]
  endif
